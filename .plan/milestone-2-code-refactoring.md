...
注意: これは実際の開発で行ったリファクタリングの**記録**です。Milestone 0の手順書は
最初からパッケージ構成で実装する形になっているため、ゼロから追う場合このマイルストーンは
「なぜこの構成にしたか」の背景説明として読み、実作業はスキップして構いません
（その場合は Milestone 1 → Milestone 3 と進む）。
