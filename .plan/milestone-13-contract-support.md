...
### 状態変更は必ずトランザクション経由にする

デプロイやstate changingな呼び出しをRPCから直接 `c.State` に反映すると、
ブロックに記録されない状態変更が生まれ、stateRoot・永続化・P2P同期・リオーグの
すべてと矛盾します。実装時は次の形にします。

- `call`（読み取り専用）: ブロックを作らず、現在のstateに対して実行して結果だけ返す
- `deployContract` / state changingな呼び出し: トランザクションとしてmempoolに入れ、
  `mineBlock` でブロックに取り込まれた時点で状態を変更する

以下の実装例のうち、RPCハンドラが直接stateを書き換えている箇所
（deployContract内のnonce加算など）は動作確認用の第一歩であり、
最終的にはトランザクション経由に置き換えてください。
...
func DeployContract(contractType string, deployer string, deployerNonce uint64, state *State) (string, error) {
    if _, exists := contractRegistry[contractType]; !exists {
        return "", errors.New("unknown contract type")
    }
    
    // 新しいコントラクトアカウントを作成（デプロイヤー+nonceから決定的に）
    address := generateContractAddress(deployer, deployerNonce)
...
func GetContract(state *State, address string) (Contract, error) {
    ...
    // 重要: 生成したインスタンスに自分のアドレスを教える。
    // これを忘れると GetStorage/SetStorage が空のaddrでstorageを引いてしまい、
    // 全コントラクトが同じ（存在しない）storageを共有する事故になる。
    instance := constructor()
    if c, ok := instance.(interface{ setAddress(string) }); ok {
...
// generateContractAddress はデプロイヤーのアドレスとnonceから決定的に生成する。
// Ethereumも同じ考え方（keccak256(rlp(sender, nonce))の下位20バイト）。
// 時刻や乱数を使うと、ブロック再適用・リオーグ・複数ノードでの再実行のたびに
// 異なるアドレスが生成され、ノード間で状態が一致しなくなる。
func generateContractAddress(deployer string, nonce uint64) string {
    data := fmt.Sprintf("%s|%d", deployer, nonce)
...
// GetContract から呼ばれる（KeyValueStore / SimpleToken にも同様に実装する）
func (c *Counter) setAddress(addr string) {
    c.addr = addr
}
...
case "deployContract":
    ...
    deployer := c.State[params.From]
    address, err := contract.DeployContract(params.ContractType, params.From, deployer.Nonce, c.State)
    if err != nil {
        return nil, rpcInvalidParams(err)
    }
...
func TestCounterContract(t *testing.T) {
    ...
    // コントラクトをデプロイ
    address, err := DeployContract("Counter", "0xalice", 0, state)
...
func TestKeyValueStoreContract(t *testing.T) {
    ...
    address, err := DeployContract("KeyValueStore", "0xalice", 0, state)
    require.NoError(t, err)
...
func TestSimpleTokenContract(t *testing.T) {
    ...
    address, err := DeployContract("SimpleToken", "0xalice", 0, state)
    require.NoError(t, err)
...
## 次のステップ
Contract風機能が実装できたら、Milestone 14でEthereum形式への接近を行います。これにより、本物のEthereumとの差分を理解し、段階的に近づけていきます。
...
Chainの状態は現在 `State map[string]account.Account` なので、コントラクト導入時は
`Contracts map[string]ContractAccount` を併設する（もしくは両者を包むcontract.State型に
ChainのStateを置き換える）必要があります。また、**stateRootの計算にコントラクトの
storageも含めること**。含めないと、コントラクト状態をいくら改ざんしてもstateRootに
現れず、状態コミットメントとして機能しなくなります。